...
本文记录 `reference_version` 对应的历史变更，旧路径按当时版本保留。后续包迁移将 `serializer.configured` 改为 `serializer.schema`；当前替代类位于 `schema.type.BasicTypeResolver` 和 `schema.parser.XmlSchemaParser`。
