...
public abstract class Basic<V extends Comparable<V>> implements Comparable<Basic<V>> {
    ...
    public static int reflectForSize(Class<? extends Basic<?>> basicClass)
    {
        ...
        try
        {
            Constructor<? extends Basic<?>> constructor = basicClass.getConstructor(ByteBuf.class, ByteOrder.class);
            return constructor.newInstance(fillingBuf, ByteOrder.nativeOrder()).size();
        }
        catch (InvocationTargetException error)
        {
            Throwable cause = error.getCause();
            if (cause instanceof TooLessBytesException tooLessBytes) throw tooLessBytes;
            throw new SerializeException("basic [" + basicClass + "] instantiate failed", cause);
        }
        catch (ReflectiveOperationException error)
        {
            throw new SerializeException("basic [" + basicClass + "] instantiate failed", error);
        }
        finally
        {
