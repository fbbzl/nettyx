...
import org.fz.nettyx.serializer.annotated.generator.StructAccessorFactory;
...
public class AnnotatedStructContext {
    ...
    static final InternalLogger log = InternalLoggerFactory.getInstance(AnnotatedStructContext.class);

    public AnnotatedStructContext(String... basePackages)
    {
        // will scan all packages if user do not assign
        this.basePackages = defaultIfEmpty(removeNull(basePackages), ALL_PACKAGES);

        synchronized (AnnotatedStructContext.class) {
    ...
    protected Set<Class<?>> classesForScan()
    {
        ...
        Filter<Class<?>> scanCondition = AnnotatedStructContext::isScannableClass;
